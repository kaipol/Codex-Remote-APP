...
export function routeNotification(method:string,params:unknown):RoutedEvent|undefined {const p=record(params),thread=String(p.threadId??record(p.thread).id??'');if(!thread)return undefined;const turn=String(p.turnId??record(p.turn).id??'')||undefined;const item=record(p.item);const itemId=String(p.itemId??item.id??'')||undefined;let type:EventType|undefined,content:string|undefined,metadata:Record<string,unknown>={method,turn_id:turn,item_id:itemId};
 ...
 else if(method==='item/completed'||method==='item/started'){const kind=String(item.type??'');metadata.item_type=kind;metadata.phase=method.endsWith('started')?'started':'completed';const clientUserMessageId=String(item.clientId??item.clientUserMessageId??'')||undefined;if(kind==='agentMessage'){if(method==='item/started')return undefined;type='assistant_message';content=String(item.text??'')}else if(kind==='userMessage'){if(method==='item/started')return undefined;type='user_message';const parsed=parseUserInput(item.content??item.text,item.images);content=cleanConversationText(parsed.content);if(parsed.references.length)metadata.references=parsed.references;if(clientUserMessageId)metadata.client_id=clientUserMessageId}else if(kind==='commandExecution'){type='command_execution';content=String(item.aggregatedOutput??'');metadata={...metadata,command:item.command,status:item.status,exit_code:item.exitCode,cwd:item.cwd}}else if(kind==='fileChange'){type='file_change';metadata={...metadata,changes:item.changes,status:item.status}}else if(kind==='reasoning'||kind==='plan'){type='reasoning_status';content=text(item.summary??item.text??item.content)}else if(kind==='mcpToolCall'||kind==='collabAgentToolCall'){type='tool_call';content=text(item.result??item.error);metadata={...metadata,server:item.server,tool:item.tool,status:item.status,arguments:item.arguments,duration_ms:item.durationMs}}else if(kind==='webSearch'){type='web_search';content=String(item.query??'');metadata={...metadata,action:item.action}}}
 else if(method==='thread/started'){const started=record(p.thread);type='session_updated';metadata.status='started';metadata.cwd=started.cwd??p.cwd;metadata.title=started.name??started.preview}
 else if(method==='thread/status/changed'){type='session_updated';metadata.status=p.status}
 else if(method==='thread/name/updated'){type='session_updated';metadata.title=p.threadName}
