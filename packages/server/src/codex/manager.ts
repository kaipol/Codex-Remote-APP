...
export class CodexManager extends EventEmitter {
 ...
 async models():Promise<ModelOption[]>{const local=await discoverModels(this.config.codexHome);try{await this.ready();const remote:ModelOption[]=[];const seenModels=new Set<string>(),seenCursors=new Set<string>();let cursor:string|null=null;do{const r:any=await this.rpc.request(METHODS.modelList,{cursor,limit:100,includeHidden:false});for(const raw of Array.isArray(r.data)?r.data:[]){const item=modelOption(raw);if(!item||seenModels.has(item.model))continue;seenModels.add(item.model);remote.push(item)}const next=typeof r.nextCursor==='string'&&r.nextCursor?r.nextCursor:null;if(!next||seenCursors.has(next)){cursor=null}else{seenCursors.add(next);cursor=next}}while(cursor);return remote.length?mergeBy(local,remote,x=>x.model):local}catch{return local}}
