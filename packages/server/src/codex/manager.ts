...
import { JsonRpcClient,RpcUnavailableError } from './json-rpc-client.js';
...
import { discoverApps,discoverModels,discoverSkills } from '../local-capabilities.js';
...
export class CodexManager extends EventEmitter {
 ...
 async read(id:string){await this.ready();return (await this.rpc.request<ThreadResponse>(METHODS.threadRead,{threadId:id,includeTurns:true})).thread}
 ...
 startTurn(id:string,input:UserInput[]|string,runtime:RuntimeConfig|string={},clientId?:string){const items=typeof input==='string'?[{type:'text' as const,text:input}]:input;const config=typeof runtime==='string'?{}:runtime;const requestId=typeof runtime==='string'?runtime:clientId;return this.serial(id,async()=>{await this.resume(id);if(this.active.has(id))throw Object.assign(new Error('thread already has an active turn'),{status:409});const result=await this.rpc.request<TurnResponse>(METHODS.turnStart,{threadId:id,clientUserMessageId:requestId,input:items.map(item=>item.type==='text'?{...item,text_elements:[]}:item),...turnRuntime(config)});if(this.completed.get(id)!==result.turn.id)this.active.set(id,result.turn.id);return {thread_id:id,turn_id:result.turn.id,status:'started'}})}
 ...
 async models():Promise<ModelOption[]>{return discoverModels(this.config.codexHome)}
...
function normalizeEffort(v:unknown):CodexDefaults['effort']{const value=valueOf(v);return value&&['none','minimal','low','medium','high','xhigh'].includes(value)?value as CodexDefaults['effort']:undefined}
