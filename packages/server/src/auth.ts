...
export class AuthService{
 ...
 refresh(deviceId:string,refresh:string):AuthTokens|undefined{const d=this.store.device(deviceId);if(!d)return;const a=Buffer.from(d.refresh_hash);const b=Buffer.from(hash(refresh));if(a.length!==b.length||!timingSafeEqual(a,b))return;const next=randomBytes(32).toString('base64url');this.store.rotateDevice(deviceId,hash(next));return this.tokens(deviceId,next)}
 verify(token:string){return jwt.verify(token,this.config.secret,{issuer:'codex-remote',audience:'codex-remote-web'}) as {sub:string;type:string}}
