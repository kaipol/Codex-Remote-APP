...
describe('event projection',()=>{
  ...
  it('ignores empty user events without references',()=>{let state:ProjectedThread={messages:[],events:[],activeTurn:false};state=mergeBridgeEvent(state,{...event('user_message','',1),metadata:{item_id:'empty-user'}});expect(state.messages).toHaveLength(0);expect(state.events).toHaveLength(1)});
