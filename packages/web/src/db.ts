...
export async function cacheSessions(items:Session[]){await db.transaction('rw',db.sessions,async()=>{await db.sessions.clear();if(items.length)await db.sessions.bulkPut(items)})}
...
export async function streamId(){return (await db.meta.get('stream_id'))?.value||''}
export async function setStreamId(value:string){if(value)await db.meta.put({key:'stream_id',value})}
