...
defineProps<{ session: Session | null; activeTurn: boolean; occupied?: boolean }>();
...
<template>
  <header class="thread-header">
    ...
    <div class="thread-title">
      ...
      <small v-if="session"><span class="status-dot" :class="{ online: session.status === 'active', occupied: occupied }"></span>{{ occupied ? '正被本机 Codex 占用' : activeTurn ? 'Codex 正在工作' : session.cwd }}</small>
    </div>
