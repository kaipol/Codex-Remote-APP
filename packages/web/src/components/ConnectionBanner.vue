...
defineProps<{online:boolean;ws:string;appServer:string;pending:number}>();
</script>
<template>
  <div v-if="!online||ws!=='connected'||appServer==='error'||pending" class="connection-banner" :class="{warning:online,error:appServer==='error'}" role="status">
