...
手机端可在本地端服务未运行时查看缓存的历史对话，无需服务器在线：

- 在设置页面设置"离线访问密码"（至少 8 位，仅以 PBKDF2-SHA256 哈希存储于本设备 IndexedDB，不经过服务器）。
- 配对页底部会出现"查看缓存对话（离线）"入口，输入离线密码即可进入只读模式浏览已同步的会话和消息。
- 离线模式下无法创建会话、发送消息或审批；所有写入操作被禁用，仅允许浏览和搜索。
- 离线模式定期探测服务器是否恢复；一旦检测到服务器上线，顶部横幅会出现"点击重连"按钮，一键回到在线模式。

离线查看的数据来自 IndexedDB 中在线时同步的缓存，可能不是最新内容。
...
- 离线缓存访问：手机端可设置独立的"离线访问密码"（PBKDF2-SHA256 哈希存储，仅本地校验）。本地端服务未运行时，配对页会显示"查看缓存对话（离线）"入口，输入此密码即可查看 IndexedDB 中缓存的历史会话和消息，无需服务器在线。离线模式为只读，不支持发送、审批、创建或重命名；服务器恢复后 ConnectionBanner 会提示重连。
